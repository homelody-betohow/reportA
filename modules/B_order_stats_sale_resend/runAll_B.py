    ...
    for idx, script in enumerate(scripts, start=1):
        ...
        if code == 0:
            ...
            if _is_b5(script):
                ...
                print(f"{Color.YELLOW}  - 优先：桌面\\MANO-MF 尾程.xlsx（COMMF 与 OHPAMF 需同时补）{Color.RESET}")
                print(f"{Color.YELLOW}  - 本机兜底：runtime\\local\\mano_mf_fee.json{Color.RESET}")
                print(
                    f"{Color.GREEN}    items 字段：映射站点 / SKU / SKU-站点识别码 / 单个-MF-派送费{Color.RESET}"
                )
                print(
                    f"{Color.YELLOW}  - B5 会把未命中记录直接追加进 mano_mf_fee.json；"
                    f"填「单个-MF-派送费」后重跑即可{Color.RESET}"
                )
            ...
            if _is_b6(script):
                ...
                print(f"\n{Color.YELLOW}【如果发现空值】{Color.RESET}")
                print(f"{Color.YELLOW}  - 优先：欧洲平台定价表（\\\\Betohow\\数据报表\\2-定价表）{Color.RESET}")
                print(f"{Color.YELLOW}  - 本机兜底：runtime\\local\\non_mf_fee.json{Color.RESET}")
                print(
                    f"{Color.GREEN}    items 字段：派送费-映射分类 / SKU / SKU-站点识别码 / 映射-单个-定价派送费{Color.RESET}"
                )
                print(
                    f"{Color.YELLOW}  - B6 会把未命中记录直接追加进 non_mf_fee.json；"
                    f"填「映射-单个-定价派送费」后重跑即可{Color.RESET}"
                )
                print(f"\n{Color.YELLOW}【操作】{Color.RESET}")
                print(f"{Color.YELLOW}  - 确认数据无误后，按回车键继续{Color.RESET}")
                print(f"{Color.YELLOW}{'=' * 80}{Color.RESET}\n")
