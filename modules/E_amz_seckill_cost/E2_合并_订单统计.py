...
# 表2
# TODO 文件路径！！！
guang_gao_path = fr'{DESKTOP_ROOT}\{folder_name}{shared_date}\秒杀\(处理完成)秒杀数据-{shared_date}.xlsx'
guang_gao_df = pd.read_excel(guang_gao_path)

# 以SKU-站点识别码为键进行合并，选择左连接（left join），这样可以确保表1的所有数据都被保留
result_df = pd.merge(main_file_df, guang_gao_df[['SKU-站点识别码', '秒杀费']], on='SKU-站点识别码', how='left')
...
# 确保所有期望的列都存在
expected_columns = list(main_file_df.columns) + ['秒杀费']
for col in expected_columns:
    if col not in result_df.columns:
        result_df[col] = None  # 如果列不存在，添加该列并填充为 None

# 表1中没有和表2相同的SKU-站点识别码，将新增的两列在对应行单元格填充为0
result_df[['秒杀费']] = result_df[['秒杀费']].fillna(0)

# 去除 整张表 的前后空格
for col in result_df.columns:
    result_df[col] = result_df[col].apply(lambda x: x.strip() if isinstance(x, str) else x)

# 重新排序，确保列的顺序符合要求
result_df = result_df[expected_columns]

# 保存结果到新的 Excel 文件
output_path = main_file_path.replace('已完成-9', '已完成-10')
