...
# 重命名
main_file_df_1 = main_file_df_1.rename(columns={'映射二级分类': '二级分类'})
# 映射 三级分类
...
main_file_df_2 = main_file_df_2.rename(columns={'映射三级分类': '三级分类'})

# 映射产品状态
# 拆分数据  平台 是否包含 AMAZON
df_amazon = main_file_df_2[main_file_df_2['平台'].str.contains('AMAZON', case=False, na=False)]
df_not_amazon = main_file_df_2[~main_file_df_2['平台'].str.contains('AMAZON', case=False, na=False)]
# 映射 AMAZON 的 产品状态
...
# 合并数据
main_file_df_3 = pd.concat([df_amazon_1, df_not_amazon_1]).reset_index(drop=True)
...
main_file_df_4 = main_file_df_4.rename(columns={'映射运营模式': '运营模式'})
# 映射 供应商
main_file_df_5 = sku_mappings(
    main_df=main_file_df_4,
    main_sku='SKU',
    map_sku_path=product_map_sku_path,
    map_old_sku="SKU",
    map_new_sku="供应商",
...
main_file_df_5 = main_file_df_5.rename(columns={'映射供应商': '供应商'})
...
# 分销 = 是 → 产品状态、二级分类、三级分类 = 分销
main_file_df_5.loc[main_file_df_5['分销'] == '是', ['产品状态', '二级分类', '三级分类']] = '分销'
...
print(f"{Color.YELLOW}[注意]检查————产品状态、三级分类、二级分类、供应商、运营模式，是不是都有了，没有的部分手动去判断、填写进去！！！{Color.RESET}")
