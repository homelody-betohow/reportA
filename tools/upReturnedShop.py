...
用法：
  python tools/upReturnedShop.py
  python tools/upReturnedShop.py --dry-run
  python tools/upReturnedShop.py --limit 100
"""
...
def cell_str(v: Any, maxlen: int | None = None) -> str | None:
    """归一化单元格为非空字符串；空值返回 None，可选截断长度。"""
    if v is None:
        return None
...
def _build_updates(
    ...
    for ret_col, shop_col, maxlen in _FILL_MAP:
        ...
        if val is not None:
            updates[ret_col] = val
    # 未能补上 platform 则整行不写，避免只改附属字段却仍被下次查出
