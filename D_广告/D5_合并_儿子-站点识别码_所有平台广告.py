    ...
    for file_path in file_paths:
        ...
        if 'DLZ' in file_path:
            df = pd.read_excel(file_path, sheet_name='广告花费-欧元',
                               usecols=['SKU', '站点', '映射平台', '儿子-站点识别码', '儿子-平台识别码',
                                        '广告费(非AMZ)'])
        else:
...
# SKU、儿子-站点识别码、儿子-平台识别码 去掉尾缀 -1、-2、-3、-4、-5、-6、-7、-8、-AT、--5；去掉前后空格
cols = ['SKU', '儿子-站点识别码', '儿子-平台识别码']
for col in cols:
    merged_df[col] = merged_df[col].str.strip().str.replace(r'(-[1-8]|-AT|--5)$', '', regex=True)

merged_df = merged_df.rename(columns={'映射平台': '平台'})
# 按照 '儿子-站点识别码' 列进行分组，并对 '广告费(非AMZ)' 列进行汇总
grouped_df = merged_df.groupby('儿子-站点识别码').agg({
...
grouped_df = grouped_df[
    ['SKU', '站点', '平台', '儿子-站点识别码', '儿子-平台识别码', '广告费(非AMZ)']]
# 将所有结果写入
