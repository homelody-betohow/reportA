"""读取钉钉在线表格「店铺信息」，并回写 ``platform_shop`` 白名单字段。
...
用法（项目根目录）::
    ...
    python app/ding-disk/platform_shop_pull.py
    python app/ding-disk/platform_shop_pull.py --dry-run

    # 只更新部分字段 / 指定店铺编码
    python app/ding-disk/platform_shop_pull.py --up-field ops_owner
    python app/ding-disk/platform_shop_pull.py --hash ecdfa4883185ebf5...
    python app/ding-disk/platform_shop_pull.py --hash ecdfa488... --up-field vat_rate --dry-run

    # 只读（不写库）
    python app/ding-disk/platform_shop_pull.py --no-update --preview 20
    python app/ding-disk/platform_shop_pull.py --list-sheets
    python app/ding-disk/platform_shop_pull.py --all --no-update --out-dir ./runtime/local/ding_shop
"""
...
# 白名单：仅允许更新这些字段；默认全部更新（不含匹配键 shop_hash）
...
def fetch_existing_values(hashes: Sequence[str], db_col: str) -> Dict[str, str]:
    """批量读取 platform_shop 当前字段值（按 shop_hash）。"""
    if not hashes:
        return {}
    result: Dict[str, str] = {}
    db = get_db_manager()
...
def apply_field_updates(
    ...
    *,
    ...
    to_update: List[Tuple[str, str]] = []  # (value, shop_hash)
    for shop_hash, value in pairs.items():
        if shop_hash not in existing:
            stats.missing_in_db += 1
            continue
        if existing[shop_hash] == value:
            stats.unchanged += 1
            continue
        to_update.append((value, shop_hash))
...
def update_fields_from_df(
    ...
    *,
    ...
    """依次更新白名单字段；缺列跳过。``hashes`` 非空时仅更新这些店铺编码。"""
    ...
    for key in field_keys:
        ...
        if stats.invalid_value:
            print(
                f"[WARN] {key}: 无法识别的状态值跳过 {stats.invalid_value} 条"
                f"（期望: 正常/停用 或 1/0）",
                file=sys.stderr,
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="读取钉钉「店铺信息」表格，默认回写 platform_shop 白名单全部字段",
        ...
        epilog=(
            ...
            "  python app/ding-disk/platform_shop_pull.py --dry-run\n"
            "  python app/ding-disk/platform_shop_pull.py --up-field ops_owner\n"
            "  python app/ding-disk/platform_shop_pull.py --hash <shop_hash>\n"
            "  python app/ding-disk/platform_shop_pull.py --no-update --preview 20\n"
        ),
    )
    ...
    parser.add_argument(
        ...
        dest="hashes",
        default=None,
        help="仅更新指定店铺编码 shop_hash（可重复）；默认更新表格中全部行",
    )
    parser.add_argument(
        "--up-field",
        action="append",
